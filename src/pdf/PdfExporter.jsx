...
export default function PdfExporter({ token, onClose }) {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Erro na busca (HTTP ${res.status})`)
  ...
  return (
    <div className="qe-shell qe-shell--embedded pdf-exporter">
      ...
      <p className="pdf-exporter-hint">
        Filtre as questões e escolha um nome. Ao gerar, o navegador baixa
        dois PDFs: um com a lista para os alunos e outro com o gabarito.
      </p>

      <section className="qe-section">
        <h3 className="qe-section-title">Filtros</h3>

        <div className="pdf-exporter-grid">
          <label className="pdf-exporter-field pdf-exporter-field--full">
            <span className="pdf-exporter-field-label">Nome da lista (opcional)</span>
            <input
              className="qe-input"
              type="text"
              value={name}
          ...
          <label className="pdf-exporter-field">
            <span className="pdf-exporter-field-label">Dificuldade (N ou N-M)</span>
            <input
              className="qe-input"
              type="text"
              value={difficulty}
          ...
          <label className="pdf-exporter-field">
            <span className="pdf-exporter-field-label">Quantidade (máx 100)</span>
            <input
              className="qe-input"
              type="number"
              min="1"
        ...
        {error && <p className="qe-error" style={{ marginTop: '1rem' }}>{error}</p>}
